...
export default function HttpCat() {
  ...
  useEffect(() => {
    const img: any = document.getElementById("mainImage");
    const errorMsg = document.getElementById("errorMsg");

    const httpCodeStatus: any = codeStatus();
    if (httpCodeStatus[httpCode] == undefined) {
      img.src =
        "https://senscast.org/wp-content/uploads/2020/03/deuruim8-1400.png";
  ...
  return (
    <section>
      <h1 className="mb-10 text-5xl font-bold">httpCat</h1>
      <div className="mb-4">
        Enter a status code and see if the HTTP Cat Api has it covered
      </div>
      <input
        type="number"
        id="username"
        className="placeholder-gray-600 bg-gray-800 text-gray-400
            ...
            mb-8
            "
        onChange={(e: any) => setHttpCode(e.target.value)}
        placeholder="Http Code"
      />
      <div id="errorMsg" className="errorMsg">
        {`The status code ${httpCode} is not covered on this API`}
      </div>
      <div>
        <img
          id="mainImage"
